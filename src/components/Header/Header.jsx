...
const HeaderComponent = memo(() => {
  ...
  const isTovariPage = location.pathname === '/tovari';
  ...
  // Модифицируем эффект для отслеживания скролла
  useEffect(() => {
    if (!isTovariPage) {
      setIsScrolled(false);
      return;
  ...
  return (
    <>
      <header className={`${s.header} ${isTovariPage ? s.headerTovari : ''} ${isScrolled ? s.fixed : ''}`}>
      ...
      {isTovariPage && <div className={s.headerSpacer} />}
    </>
  );
});
